...
function ProjectCard({ project, onDelete }: { project: ReviewData, onDelete: (e: React.MouseEvent, id: string) => void }) {
  const targetDate = project.completed_at || project.created_at;
  ...
  if (project.report) {
    ...
    const legacy = project.report as LegacyReport;
    const sSecA = project.report.security_archi?.score || legacy.security?.score || 0;
    const sSecC = project.report.security_code?.score || legacy.code_quality?.score || 0;
  ...
  let statusColor = "bg-blue-500 animate-pulse";
  if (isFailed) statusColor = "bg-red-500";
  if (isCompleted) statusColor = "bg-emerald-500";

  let statusMsg = "Analyse en cours...";
  if (isCompleted) statusMsg = project.report?.architecture?.summary || "";
  if (isFailed) statusMsg = project.error_message || "";

  return (
    <Link href={`/reviews/${project.id}`} className="group">
